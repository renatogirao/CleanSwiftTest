...
class ServiceInteractor {
    ...
    let headers: HTTPHeaders = [
     ...
     func loadCards(endpoint: endpoints, filterType: filterManager ,onComplete: @escaping (CardsResponse?) -> Void ) {
        
        let finalURL = "\(endpoint)+\(filterType)"
        AF.request(finalURL, headers: headers).responseJSON { (response) in
